...
nonisolated enum ProFeature: String, Identifiable, Sendable {
    ...
    var blurb: String {
        ...
        case .storage:        String(localized: "R2 对象存储、D1 数据库与 KV 键值管理属于 OneCFCloud Pro。")
        case .workerTail:     String(localized: "Workers 实时日志与灵动岛 Live Activity 属于 OneCFCloud Pro。")
        case .waf:            String(localized: "查看与启停 WAF 自定义规则属于 OneCFCloud Pro。")
        case .tunnel:         String(localized: "Cloudflare Tunnel 的查看与管理（新建隧道、公共主机名路由）属于 OneCFCloud Pro。")
        case .analyticsRange: String(localized: "7 天与 30 天流量分析属于 Pro；24 小时视图永久免费。")
        case .snippets:       String(localized: "查看与管理域名的边缘 Snippets（JS 代码片段）属于 OneCFCloud Pro。")
        case .workerSecrets:  String(localized: "管理 Workers 的环境变量与密钥属于 OneCFCloud Pro。")
        case .workerTriggers: String(localized: "管理 Workers 的 Cron 定时触发器属于 OneCFCloud Pro。")
        case .workerRoutes:   String(localized: "管理 Workers 的子域、自定义域与路由属于 OneCFCloud Pro。")
        case .cacheRules:     String(localized: "按 URL 自定义边缘/浏览器缓存 TTL、绕过缓存等缓存规则属于 OneCFCloud Pro。")
        case .pages:          String(localized: "查看与管理 Cloudflare Pages 项目和部署（重试 / 回滚 / 删除、构建配置）属于 OneCFCloud Pro。")
        case .loadBalancing:  String(localized: "负载均衡器、源站池与健康监测的查看与管理属于 OneCFCloud Pro。")
        case .bulkRedirects:  String(localized: "批量 URL 重定向列表与条目的查看与管理属于 OneCFCloud Pro。")
        case .auditLog:       String(localized: "查看账号最近 30 天的审计日志（谁在何时改了什么）属于 OneCFCloud Pro。")
        case .emailRouting:   String(localized: "管理域名的邮件路由规则与目的地址属于 OneCFCloud Pro。")
        case .rateLimit:      String(localized: "查看与管理限速规则属于 OneCFCloud Pro。")
        case .zeroTrust:      String(localized: "查看 Zero Trust Access 应用与 Gateway 策略属于 OneCFCloud Pro。")
        case .trafficMap:     String(localized: "按国家/地区在世界地图上查看请求与威胁的地理分布属于 OneCFCloud Pro。")
        case .aiInsights:     String(localized: "用设备端 AI 一句话总结本期流量的增长、异常与主要来源（离线、免费、不出设备）属于 OneCFCloud Pro。")
        case .aiDNS:          String(localized: "用自然语言一句话生成 DNS 记录（如「给 blog 加个指向 1.2.3.4 的 A 记录」），设备端离线属于 OneCFCloud Pro。")
        case .filesApp:       String(localized: "把 R2 存储桶挂进系统『文件』App，像 iCloud 云盘一样浏览、读写、用任意 App 打开，属于 OneCFCloud Pro。")
        case .queues:         String(localized: "查看与管理 Cloudflare Queues（新建 / 删除、暂停投递、清空消息、改保留期与延迟）属于 OneCFCloud Pro。")
        case .aiGateway:      String(localized: "查看与管理 AI Gateway（新建 / 删除网关，配置缓存、限速与日志）属于 OneCFCloud Pro。")
        case .durableObjects: String(localized: "查看 Durable Objects 命名空间，并浏览其中的对象实例属于 OneCFCloud Pro。")
        case .workersAI:      String(localized: "浏览 Workers AI 模型目录，并试运行文本生成模型属于 OneCFCloud Pro。")
        case .hyperdrive:     String(localized: "查看与管理 Hyperdrive 数据库加速配置（缓存设置、改源连接、新建 / 删除）属于 OneCFCloud Pro。")
        }
    }
    ...
    var body: some View {
        ContentUnavailableView {
            ...
            Button {
                ...
                Label(String(localized: "了解 OneCFCloud Pro"), systemImage: "sparkles")
            }
